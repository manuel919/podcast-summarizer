...
class WhisperTranscriber:
    ...
    def chunk(self, audio_path):
        ...
        if file_size > 25 * 1024 * 1024:
            print(
                f'↪ The audio file is too large: {(file_size / 1024 / 1024):.2f} MB (>25MB), chunking...'
            )
            ...
            for i in range(0, len(audio), chunk_length_ms):
                chunk_name = (
                    f"downloads/whisper/{file_name.split('.')[0]}_{i // chunk_length_ms}.mp3"
                )
        ...
        for audio in audio_list:
            ...
            if not os.path.exists(transcript_path):
                ...
                with open(transcript_path, "w") as f:
                    ...
                    print(
                        f"\t\t↪ saved transcript to {audio.split('.')[0]}.txt (words: {len(transcript.split())})"
                    )
            else:
